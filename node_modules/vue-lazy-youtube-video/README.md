...
| `thumbnailListeners` | `false`  | `Record<string, Function>`     |                                                                                                                               | Listeners that will be attached to the preview thumbnail                                                                                                                                                                                                  |
...
> ⚠️ **Note**, that when `button` slot is passed and this slot contains `<button></button>` element, ones should not to forget to add `aria-label` attribute. Also, if that button do not contain `.y-video-button` class, all default styles will be lost, so style concerns is up to developer.
